"""
Модуль с assert-функциями для тестирования API файлов.
Содержит проверки для создания, получения, удаления файлов,
а также обработки валидационных ошибок.
"""
...
def assert_get_file_with_incorrect_file_id_response(actual: ValidationErrorResponseSchema):
    """
    Проверяет, что ответ на запрос получения файла с некорректным file_id 
    соответствует ожидаемой валидационной ошибке.

    Ожидаемая ошибка возникает, когда вместо валидного UUID передается 
    строка, не соответствующая формату UUID (например, "incorrect-file-id").
